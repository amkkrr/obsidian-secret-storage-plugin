...
- **密钥搜索**
  - 「获取 / 删除 / 插入占位符」三个选择器改为模糊搜索（`FuzzySuggestModal`），输入即过滤、键盘选择。
  - 设置页密钥列表顶部新增搜索框，实时过滤密钥 ID。
  - 搜索只匹配密钥 ID，不触碰密钥值（安全）。
...
| 获取密钥 (Get Secret) | 搜索并复制已存密钥（模糊搜索） | 全部 |
| 搜索密钥并复制 (Search & Copy Secret) | 快速搜索密钥并复制到剪贴板 | 全部 |
