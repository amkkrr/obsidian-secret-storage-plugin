...
- wideTable
url: null
permalink: docs/secret-storage-v1
---

## Secret Storage Demo 插件 — 移动端支持设计文档（v1）
...
5. **不引入 Node/Electron 依赖：** 本插件本就未用任何 Node/Electron API，满足 `isDesktopOnly: false` 的前置条件。
...
**`main.ts`**
- `initStorageProvider`：若设置中 `storageMode === "local"` 但 `app.secretStorage` 不可用（移动端低版本），自动切到 `sync` 并 Notice 提示"当前版本不支持本地模式，已切换到同步模式"。
...
8. 提交、推送 `feature/mobile-support` 分支；打 tag 触发 CI 构建发布。
...
- **桌面模拟：** 开发者工具 Console 执行 `this.app.emulateMobile(true)`，验证模态框单列布局、剪贴板 fallback 路径、本地模式降级提示。
